...
function activate(context) {
    ...
    const runStreamlit = (content, isFile) => {
        if (!content) {
        ...
        vscode.window.showInformationMessage(`Running Streamlit with ${isFile ? "file" : "selected text"}`);
        ...
        (0, child_process_1.exec)(command, { cwd: context.extensionPath }, (error, stdout, stderr) => {
            if (error) {
        ...
        try {
            ...
            child.on('close', (code) => {
                if (code !== 0) {
                    ...
                    vscode.window.showErrorMessage(`Docker Compose failed. Check output for details.`, 'Open Output').then(selection => {
    ...
    function getScriptsPath(context) {
    ...
    const creditrisk = () => {
        ...
        vscode.window.showInformationMessage(`Running Streamlit for Credit Risk Analysis Testing`);
        // Pass the content as a single argument
    ...
    const regulatorycompliance = (filePath) => {
        if (!filePath) {
    ...
    let command1 = vscode.commands.registerCommand(`finsure.fileOption${1}`, (fileUri) => {
    ...
    let command2 = vscode.commands.registerCommand(`finsure.fileOption${2}`, (fileUri) => {
        // Read the entire file content
        const filePath = fileUri.fsPath;
        const fileContent = fs.readFileSync(filePath, "utf-8");
        creditrisk();
    ...
    let command3 = vscode.commands.registerCommand(`finsure.fileOption${3}`, (fileUri) => {
        // Pass the file path directly (no need to read content here)
        const filePath = fileUri.fsPath;
        regulatorycompliance(filePath);
    });
    context.subscriptions.push(command3);
    let command4 = vscode.commands.registerCommand(`finsure.fileOption${4}`, () => {
        chatbottesting();
    });
    context.subscriptions.push(command4);
    let command5 = vscode.commands.registerCommand(`finsure.fileOption${5}`, () => {
        uitesting();
    });
    context.subscriptions.push(command5);
    // Register commands for Editor context menu (text selection)
    let command6 = vscode.commands.registerCommand(`finsure.selectionOption${1}`, () => {
    ...
    let command7 = vscode.commands.registerCommand(`finsure.selectionOption${2}`, () => {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
            const selection = editor.document.getText(editor.selection);
            creditrisk();
    ...
    let command9 = vscode.commands.registerCommand(`finsure.selectionOption${4}`, () => {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
            chatbottesting();
        }
    ...
    let command10 = vscode.commands.registerCommand(`finsure.selectionOption${5}`, () => {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
            const selection = editor.document.getText(editor.selection);
            uitesting();
