...
def run_news_api():
    mock_api_dir = os.path.join(os.getcwd(), "mockAPIs")
    if not os.path.isdir(mock_api_dir):
        print(f"[ERROR] Directory 'mockAPIs/' not found at {mock_api_dir}.")

    try:
        print(f"[INFO] Starting mock API server in: {mock_api_dir}")
        subprocess.run(
        ...
        print(f"[ERROR] Failed to start the mock API server: {e}")

def run_news_api_as_process():
    """
    Runs the run_news_api function as a separate process.
    """
    process = Process(target=run_news_api)
    process.start()
    print("[INFO] run_news_api is running as a separate process.")
    return process
...
#     """
#     Sends a prompt to the Gemini mock API and retrieves the generated code.
#     """
...
#             print("[INFO] Generated Code from Gemini API:")
#             print(generated_code)
#             return generated_code
#         else:
#             print("[ERROR] Gemini API returned an error status.")
#             return None
#     except requests.exceptions.RequestException as e:
...
def run_gradle_tests(class_name, project_directory):
    ...
    # gradle_path = r"D:\\gradle-8.13-all\\gradle-8.13\\bin\\gradle.bat"  # Update if needed
    # gradle_command = [gradle_path, "test", f"--tests={class_name}"]
